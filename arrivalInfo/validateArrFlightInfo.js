...
module.exports = function(sessionAttributes, slotDetails, arrivalDate, source, airline) {
  ...
  if(arrivalDate!=null && sessionAttributes.arrivalDate == null) {
    const validateResultOfDate=flightInfoManager.validateFlightDate(slotDetails, arrivalDate);
    // 이 결과가 참
    ...
    // 이 결과가 거짓
      ...
      return Promise.resolve(buildValidationResult(false, 'aaArrivalDate', 'it\'s invalid date. Please tell me date inside 7 days! If there is not your date below list, PLEASE WRITE THE DATE such as \"YYYY-MM-DD\"', validateResultOfDate.options, sessionAttributes));
    }
  }
  ...
  if(source!=null && sessionAttributes.sourceCode == null) {
    return flightInfoManager.validateAirportName(source).then(validateResultOfSource => {
      if(validateResultOfSource!=null) {
        ...
        return buildValidationResult(false, 'aaSource', `${source} is not exact city name. Please say correct name`, sessionAttributes);
      }
    });
  }
